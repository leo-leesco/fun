...
(* The file %.th  is the source file.
...
let process_positive_test command sourcedir filename : unit =
  ...
  modes |> List.iter (fun mode ->
    (* In each test, we process the source file and check if the actual output
       in [%.out] matches the expected output in [%.exp]. The messages printed
       on the error channel are logged in the file [%.err], but are not compared
       against a reference. *)
    run_and_compare
